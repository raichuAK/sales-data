...
class BusinessService{
  ...
  async getTop30Total() {
    ...
    for(let contact of contactsData) {
      ...
      for(let listingCount of countSorted) {
        if (countTilltop30 < top30Count) {
          top30listings.push(listingCount[0]);
          countTilltop30 = countTilltop30 + listingCount[1];
        } else {
          break;
        }
      }
      let top30price = 0;
      const listingsData = await Cache.getListingsData();
      for(let listingId of top30listings) {
        for (let listing of listingsData) {
          if (listingId === listing.id) {
            top30price = top30price + parseInt(listing.price, 10);
            break;
          }
        }
      }
    ...
    for(let contact of contactsData) {
      ...
      if (!prevMonYearVal) {
        monYearMap[monYear] = {
          ...
          let prevListingVal = prevMonYearVal[contact.listing_id];
          if (!prevListingVal) {
